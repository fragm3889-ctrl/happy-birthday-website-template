...
export default function NoteCard({ message, index, align = "center" }: NoteCardProps) {
      ...
      const ctx = gsap.context(() => {
        gsap.fromTo(
          cardRef.current,
          {
            y: 150,
            opacity: 0,
            rotationZ: index % 2 === 0 ? -3 : 3, // polaroid tilt
            scale: 0.9,
          },
          {
            y: 0,
            opacity: 1,
            rotationZ: index % 2 === 0 ? -1 : 1, // Rest at slight angle
            scale: 1,
            duration: 1.5,
            ease: "back.out(1.2)",
            scrollTrigger: {
  ...
  return (
    <div 
      ref={cardRef} 
      onClick={() => playPop()}
      className={`w-[90%] md:w-[60%] lg:w-[45%] p-8 md:p-12 my-[20vh] rounded-[2rem] bg-white/50 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(255,182,193,0.3)] border border-white/60 ${alignClass} transition-transform hover:scale-[1.03] active:scale-95 duration-300 cursor-pointer`}
      title="Tap me!"
    >
